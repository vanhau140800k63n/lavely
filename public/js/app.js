...
$(document).ajaxError(function (event, jqXHR, ajaxSettings, thrownError) {
  if (jqXHR.status === 401) {
    Swal.fire({
      ...
      window.location.href = "".concat(DOMAIN, "/login");
    });
  } else {
    Swal.fire({
      title: 'Lỗi hệ thống!',
...
$(document).ready(function () {
  ...
  $('#search_input').on('input', function () {
    ...
    var suggestionsBox = $('#search_suggestions');
        ...
        $.ajax({
          ...
          success: function success(response) {
            if (response.result && response.products.length > 0) {
              var suggestions = response.products.map(function (item) {
                var highlightedName = item.name.replace(new RegExp(query, 'gi'), function (match) {
                  return "<strong>".concat(match, "</strong>");
                });
                return "<li product_id=\"".concat(item.id, "\"> ").concat(highlightedName, " </li>");
              }).join('');
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkdevshop"] = self["webpackChunkdevshop"] || [];
